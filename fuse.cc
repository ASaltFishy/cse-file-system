...
{
    struct stat st;
    chfs_client::inum inum = ino; // req->in.h.nodeid;(64-bit)
    chfs_client::status ret;
                     ...
                     off_t off, struct fuse_file_info *fi)
{
    ...
    chfs_client::status ret;
    std::string buf;
    printf("\tfuse read file%lld, off:%ld, size:%ld\n", inum, off, size);
                      ...
                      struct fuse_file_info *fi)
{
    ...
    chfs_client::status ret;
    size_t written_size;
    printf("\tfuse write file%lld, off:%ld, size:%ld\n", inum, off, size);
...
fuseserver_createhelper(fuse_ino_t parent, const char *name,
                        mode_t mode, struct fuse_entry_param *e, int type, const char *link = NULL)
{
    int ret;
    ...
    if (type == extent_protocol::T_FILE)
        ...
        ret = chfs->symlink(parent, name, inum, link);
    if (ret != chfs_client::OK)
        return ret;
    e->ino = inum;
...
void fuseserver_symlink(fuse_req_t req, const char *dst, fuse_ino_t parent, const char *linkname)
{
    struct fuse_entry_param e;
    chfs_client::status ret;
    printf("fuse create symlink:%s content:%s\n",linkname,dst);
    if ((ret = fuseserver_createhelper(parent, linkname, 0, &e, extent_protocol::T_SYMLINK, dst)) == chfs_client::OK)
    {
        fuse_reply_entry(req, &e);
    }
    else
