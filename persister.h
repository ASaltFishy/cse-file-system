...
public:
    ...
    int append_log(const command &log);
    ...
    std::vector<command> &get_undolist();
...
private:
    ...
    std::vector<command> uncommitted_entries;
...
template <typename command>
int persister<command>::append_log(const command &log)
{
    // Your code here for lab2A
    if (logSize + log.getSize() <= MAX_LOG_SZ)
    {
...
template <typename command>
std::vector<command> &persister<command>::get_undolist(){
    std::ifstream *inFile = new std::ifstream();
    inFile->open(file_path_logfile, std::ofstream::binary);
    if (inFile->is_open())
    {
        printf("persist——undo: open log file successfuly!\n");
        parseEntry(inFile);
    }
    else
    {
        printf("persist——undo: log file not exist, skip parsing!\n");
    }
    inFile->close();
    // don't recover uncommitted transaction
...
{
    // Your code here for lab2A
    //snapshot the disk now (already undo)
    //overwrite, not append
    std::ofstream *outCheckFile = new std::ofstream();
    outCheckFile->open(file_path_checkpoint, std::ofstream::binary);
    im->snapshot(outCheckFile);
    outCheckFile->close();
    delete outCheckFile;

    // overwrite, not append
    std::ofstream *outFile = new std::ofstream();
    outFile->open(file_path_logfile, std::ofstream::binary);
    logSize = 0;
    for(auto it=uncommitted_entries.begin();it<uncommitted_entries.end();it++){
        it->toEntry(outFile);
        logSize+=it->getSize();
    }
    outFile->close();
    delete outFile;
    printf("checkpoint: logSize after check: %d\n",logSize);
}
...
{
    printf("parse: start parseEntry function now!\n");
    chfs_command::txid_t id;
    chfs_command::size_t oldsize, newsize;
...
void persister<command>::restore_checkpoint(inode_manager *im)
{
    // Your code here for lab2A
    std::ifstream *inFile = new std::ifstream();
    inFile->open(file_path_checkpoint, std::ofstream::binary);
    if (inFile->is_open())
    {
        im->load_snapshot(inFile);
    }
    inFile->close();
    delete inFile;
};
